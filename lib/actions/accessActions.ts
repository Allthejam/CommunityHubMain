...
export async function grantSpecialAccessAction(params: {
  ...
  try {
    ...
    // 1. Update the user's profile to grant access and set display name
    const userRef = firestore.collection('users').doc(userId);
    batch.update(userRef, { 
        'settings.hasBroadcastAccess': true,
        'broadcastDisplayName': requestData.displayName || requestData.applicantName // Fallback to applicant name
    });
