...
const communityTeamRoles = ['reporter', 'editor', 'moderator', 'broadcaster', 'administrator', 'vice-president', 'treasurer', 'secretary', 'leader', 'community-courier'];
...
const UserRow = React.memo(({ user, currentCourierId, onAppointCourier, onUnappointCourier, onUpdateStatus, onViewDetails, onEditRole, onRemove }: { 
    user: Member;
    currentCourierId: string | null;
    onAppointCourier: (userId: string) => void;
    onUnappointCourier: () => void;
    ...
    const baseRole = user.accountType === 'personal' ? 'Member' 
                     : user.accountType ? user.accountType.charAt(0).toUpperCase() + user.accountType.slice(1)
                     : 'Member';
                     
    const specificRole = user.title || (user.role ? user.role.split('-').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ') : '');
    ...
    if (specificRole && specificRole !== 'Member' && specificRole !== baseRole) {
        allRoles.add(specificRole);
    }
    
    const displayRole = Array.from(allRoles).join(' | ');
    const isCurrentCourier = user.id === currentCourierId;
    ...
    const dropdownMenuContent = (
        <DropdownMenuContent align="end">
            ...
            {user.role !== 'president' && (
             ...
             {isCurrentCourier ? (
                <DropdownMenuItem onClick={onUnappointCourier}>
                    <UserX className="mr-2 h-4 w-4" /> Unappoint Courier
                </DropdownMenuItem>
            ) : !currentCourierId ? (
                <DropdownMenuItem onClick={() => onAppointCourier(user.id)}>
                    <Truck className="mr-2 h-4 w-4" /> Appoint as Courier
                </DropdownMenuItem>
            ) : null}
            <DropdownMenuSeparator />
    ...
    const contextMenuItems = (
        <>
            ...
            {user.role !== 'president' && (
             ...
             {isCurrentCourier ? (
                <ContextMenuItem onSelect={onUnappointCourier}>
                    <UserX className="mr-2 h-4 w-4" /> Unappoint Courier
                </ContextMenuItem>
             ) : !currentCourierId ? (
                <ContextMenuItem onSelect={() => onAppointCourier(user.id)}>
                    <Truck className="mr-2 h-4 w-4" /> Appoint as Courier
                </ContextMenuItem>
             ) : null}
            <ContextMenuSeparator />
                    ...
                    <TableRow className="block md:table-row">
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="flex-1 overflow-hidden">
                                        ...
                                        <div className="flex items-center gap-2 flex-wrap">
                                            ...
                                            {isCurrentCourier && <Badge variant="secondary" className="text-xs"><Truck className="h-3 w-3 mr-1" /> Courier</Badge>}
                                        </div>
                                    </div>
                                </div>
                                <DropdownMenu>
                        ...
                        <TableCell className="hidden md:table-cell capitalize">
                            <div className="flex items-center gap-2">
                                ...
                                {isCurrentCourier && <Badge variant="secondary"><Truck className="h-3 w-3 mr-1" /> Courier</Badge>}
                            </div>
                        </TableCell>
...
export default function LeaderMembersPage() {
            ...
            snapshot.forEach(docSnap => {
                ...
                if (isActualMember) {
                    ...
                    membersData.push({
                        ...
                        joined: docSnap.data().joined?.toDate ? docSnap.data().joined.toDate() : new Date(),
                    } as Member)
                }
            });
  ...
  const handleSort = (key: keyof Member) => {
    ...
    const handleAppointCourier = async (userId: string) => {
        ...
        if (result.success) {
            toast({
                title: "Courier Appointed!",
                description: "The member has been assigned as the Community Courier.",
            });
        } else {
            toast({
                ...
                {loading ? (
                  ...
                  paginatedMembers.map((user) => (
                    <UserRow 
                        ...
                        onAppointCourier={handleAppointCourier}
                        onUnappointCourier={handleUnappointCourier}
