...
export default function MyEventsPage() {
  ...
  const eventsQuery = useMemoFirebase(() => {
    if (!userProfile?.communityId) return null;
    return query(collection(db, "events"), where("communityId", "==", userProfile.communityId));
  ...
  const handleUpdateStatus = async (eventId: string, status: CommunityEvent['status']) => {
        ...
        if (result.success) {
            toast({ title: "Status Updated", description: `Event status changed to ${status}.`});
        } else {
