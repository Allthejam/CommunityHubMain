...
import { add, differenceInDays, format, isValid } from "date-fns";
            ...
            <TableCell className="block md:table-cell text-right" data-label="Actions">
                <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" onClick={(e) => e.stopPropagation()}>
                        ...
                        <DropdownMenuItem onSelect={(e) => { e.preventDefault(); setTimeout(() => window.open(`/news/${story.id}`, '_blank'), 100); }}>
                            <Eye className="mr-2 h-4 w-4" /> View Story
                        </DropdownMenuItem>
                        
                        {canEdit && story.status !== "Archived" && <DropdownMenuItem onSelect={(e) => { e.preventDefault(); setTimeout(() => window.location.href = `/leader/news/edit/${story.id}`, 100); }}><FileEdit className="mr-2 h-4 w-4" /> Edit Story</DropdownMenuItem>}
                        
                        {(story.status !== "Archived" && story.status !== "Published") && canEdit && <DropdownMenuSeparator />}

                        {story.status === "Pending Approval" && canApprove && (
                            <>
                                <DropdownMenuItem onSelect={(e) => { e.preventDefault(); setTimeout(() => onAction(story, 'approve'), 100); }}><BadgeCheck className="mr-2 h-4 w-4" /> Approve</DropdownMenuItem>
                                <DropdownMenuItem onSelect={(e) => { e.preventDefault(); setTimeout(() => onAction(story, 'request-edit'), 100); }}><FileEdit className="mr-2 h-4 w-4" /> Request Edit</DropdownMenuItem>
                                <DropdownMenuItem className="text-destructive focus:bg-destructive/10 focus:text-destructive" onSelect={(e) => { e.preventDefault(); setTimeout(() => onAction(story, 'decline'), 100); }}>
                         ...
                         {story.status !== "Archived" && canEdit && (
                            <DropdownMenuItem onSelect={(e) => { e.preventDefault(); setTimeout(() => onAction(story, 'archive'), 100); }}>
                        ...
                        {story.status === "Archived" && canEdit &&(
                            <DropdownMenuItem className="text-destructive focus:bg-destructive/10 focus:text-destructive" onSelect={(e) => { e.preventDefault(); setTimeout(() => onAction(story, 'delete'), 100); }}>
        ...
        <CardFooter>
            <DropdownMenu>
                <DropdownMenuTrigger asChild>
                 ...
                 <DropdownMenuContent align="end" onClick={(e) => e.stopPropagation()}>
                    <DropdownMenuLabel>Actions</DropdownMenuLabel>
                    <DropdownMenuItem onSelect={(e) => { e.preventDefault(); setTimeout(() => window.open(`/news/${story.id}`, '_blank'), 100); }}><Eye className="mr-2 h-4 w-4" /> View Story</DropdownMenuItem>
...
function LeaderNewsContent() {
    ...
    const paginatedActive = React.useMemo(() => {
        const start = activePagination.pageIndex * activePagination.pageSize;
        return activeNews.slice(start, start + activePagination.pageSize);
    }, [activeNews, activePagination]);

    const activePageCount = Math.ceil(activeNews.length / activePagination.pageSize);
    ...
    const getDialogDescription = () => {
        ...
        switch(actionType) {
            case 'archive': return `${baseText} This will remove it from the public view.`;
            case 'delete': return `${baseText} This action is permanent and cannot be undone.`;
            case 'decline': return `${baseText} This will reject the submission.`;
            case 'request-edit': return `${baseText} This will send it back to the author with your notes.`;
            ...
            <div className="lg:col-span-1">
                 <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Members who can create and submit news stories.</CardDescription>
                    </CardHeader>
                    <CardContent>
                ...
                <Card>
                    ...
                    <CardContent>
                        {isMobile ? (
                         ...
                         <PaginationControls pagination={activePagination} setPagination={setActivePagination} pageCount={activePageCount} totalRows={activeNews.length} />
                    </CardContent>
                </Card>
            </div>
        </div>
        ...
        <div className="mt-8">
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>Stories that have been published and then removed.</CardDescription>
                </CardHeader>
                <CardContent>
                    <div className="rounded-md border">
                            <Table className="responsive-table">
                                            ...
                                            <TableCell className="block md:table-cell text-right" data-label="Actions">
                                                <DropdownMenu>
                                                    ...
                                                    <DropdownMenuContent align="end" onClick={(e) => e.stopPropagation()}>
                                                        <DropdownMenuLabel>Actions</DropdownMenuLabel>
                                                        <DropdownMenuItem onSelect={(e) => { e.preventDefault(); setTimeout(() => window.open(`/news/${story.id}`, '_blank'), 100); }}>
                                                            <Eye className="mr-2 h-4 w-4" /> View Original Story
                                                        </DropdownMenuItem>
                                                        <DropdownMenuSeparator />
                                                        <DropdownMenuItem className="text-destructive focus:bg-destructive/10 focus:text-destructive" onSelect={(e) => { e.preventDefault(); setTimeout(() => handleAction(story, 'delete'), 100); }}>
                                                            <Trash2 className="mr-2 h-4 w-4" />
                                                            Delete Permanently
                                                        </DropdownMenuItem>
                                                    </DropdownMenuContent>
