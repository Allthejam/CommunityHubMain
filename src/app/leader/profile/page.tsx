...
import { Settings, User, MapPinned, UserCog, Save, Shield, Siren, KeyRound, LogOut, Loader2, AlertTriangle, HelpCircle, DollarSign, Crown, Mic, Activity, BellRing, RefreshCcw, ArrowLeft, Building, Send, Info, Users as UsersIcon, Bug, PlusCircle, Pencil, Trash2, Check, ChevronsUpDown, ShieldAlert, FileEdit } from "lucide-react";
...
import { collection, onSnapshot, query, where, doc, getDoc, writeBatch, getDocs, limit, type Query } from "firebase/firestore";
import { runSavePoliceLiaison, getCommunitiesWithLiaisonStatus } from "@/lib/actions/communityActions";
...
function PlatformSettingsContent() {
  ...
  React.useEffect(() => {
    ...
    const policeQuery = query(collection(db, "users"), where("role", "==", "police-liaison-officer"));
    const unsubPolice = onSnapshot(policeQuery, (snapshot) => {
        const officers = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as TeamMember));
        setLiaisonOfficers(officers);
    });
  ...
  const isPoliceContactIncomplete = !policeContact.officerId || !policeContact.officerRank || !policeContact.stationName || !policeContact.contactEmail || !policeContact.contactPhone;

  if (loading || isUserLoading || profileLoading || communityLoading || communityProfileDocLoading) {
                ...
                <Dialog open={isPoliceLiaisonDialogOpen} onOpenChange={setIsPoliceLiaisonDialogOpen}>
                    <DialogTrigger asChild>
                        <Button variant="outline">
                            ...
                            Set or Edit Police Liaison
                        </Button>
                    </DialogTrigger>
                    <DialogContent>
                        <DialogHeader>
                            <DialogTitle>Police Liaison Details</DialogTitle>
                        </DialogHeader>
                         <div className="py-4 space-y-4">
                            <div className="grid gap-2">
                                <Label>Liaison Officer (from members) *</Label>
                                <Popover open={isMemberSearchOpen} onOpenChange={setIsMemberSearchOpen}>
                                    <PopoverTrigger asChild>
                                        <Button variant="outline" role="combobox" className="w-full justify-between">
                                            {policeContact.officerId ? liaisonOfficers.find(m => m.id === policeContact.officerId)?.name : "Select a member..."}
                                    ...
                                    <PopoverContent className="w-[--radix-popover-trigger-width] p-0">
                                        <Command>
                                            ...
                                            <CommandGroup>
                                                {liaisonOfficers.map(member => (
                                                    <CommandItem
                                                        key={member.id}
                                                        value={member.id}
                                                        onSelect={(currentValue) => {
                                                            const selected = liaisonOfficers.find(m => m.id === currentValue);
                                                            if (selected) {
                                                                setPoliceContact({
                                                    ...
                                                    >
                                                        <Check className={cn("mr-2 h-4 w-4", policeContact.officerId === member.id ? "opacity-100" : "opacity-0")} />
                                                        {member.name}
                                                    </CommandItem>
                                                ))}
                                            </CommandGroup>
                            ...
                            <div className="grid gap-2">
                                ...
                                <Input id="officer-name" value={policeContact.officerName} disabled readOnly/>
                            </div>
                            <div className="grid gap-2">
                                <Label htmlFor="officer-rank">Rank / Title *</Label>
                                <Input id="officer-rank" placeholder="e.g., PC, Sgt, CSO" value={policeContact.officerRank} onChange={(e) => setPoliceContact(p => ({ ...p, officerRank: e.target.value }))} />
                            </div>
                            <div className="grid gap-2">
                                <Label htmlFor="station-name">Station Name/Address *</Label>
                                <Input id="station-name" placeholder="e.g., Sunnyvale Police Station, 123 High St" value={policeContact.stationName} onChange={(e) => setPoliceContact(p => ({ ...p, stationName: e.target.value }))}/>
                            </div>
                            <div className="grid gap-2">
                                <Label htmlFor="contact-email">Contact Email *</Label>
                                <Input id="contact-email" type="email" placeholder="e.g., contact@sunnyvalepolice.gov" value={policeContact.contactEmail} disabled readOnly />
                            </div>
                            <div className="grid gap-2">
                                <Label htmlFor="contact-phone">Contact Phone *</Label>
                                <Input id="contact-phone" type="tel" placeholder="e.g., 101" value={policeContact.contactPhone} onChange={(e) => setPoliceContact(p => ({ ...p, contactPhone: e.target.value }))}/>
                            </div>
                        </div>
                        <DialogFooter>
                            <DialogClose asChild><Button variant="outline">Cancel</Button></DialogClose>
                            <Button onClick={handlePoliceContactSave} disabled={isPoliceContactIncomplete || isSavingPoliceContact}>
                 ...
                 {policeContact.officerId ? (
                    <div className="mt-4 p-4 border rounded-lg bg-muted/50">
                        <p><strong>Officer:</strong> {allMembers.find(m => m.id === policeContact.officerId)?.name} ({policeContact.officerRank})</p>
                        <p><strong>Station:</strong> {policeContact.stationName}</p>
                        <p><strong>Contact:</strong> {policeContact.contactEmail}, {policeContact.contactPhone}</p>
                    </div>
                ) : (
