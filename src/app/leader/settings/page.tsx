...
import { Settings, User, MapPinned, UserCog, Save, Shield, Siren, KeyRound, LogOut, Loader2, AlertTriangle, HelpCircle, DollarSign, Crown, Mic, Activity, BellRing, RefreshCcw, ArrowLeft, Building, Send, Info, Users as UsersIcon, Bug, PlusCircle, Pencil, Trash2, Check, ChevronsUpDown, ShieldAlert, FileEdit, UserX, Truck, UserPlus } from "lucide-react";
...
import { collection, onSnapshot, query, where, getDocs, limit, type Query, documentId, getDoc, writeBatch, doc } from "firebase/firestore";
...
import { savePlatformRolesAction, runHandoverLeadership, runSaveCommunityTeamPermissions, addExistingMemberToCommunity, removeMemberFromCommunityAction, updateMemberRoleAction, getStaffProfile, saveStaffProfileAction, promoteToStaffAction, demoteStaffAction } from "@/lib/actions/teamActions";
...
import { appointCommunityCourierAction, unappointCommunityCourierAction } from "@/lib/actions/courierActions";
...
const permissionGroups = [
    { 
        ...
        permissions: [
            ...
            { key: 'viewAnalytics', label: 'View Page: Analytics' },
            { key: 'viewUsers', label: 'View Page: Users' },
            { key: 'viewCommunities', label: 'View Page: Communities' },
            { key: 'viewTeam', label: 'View Page: Team' },
            { key: 'viewFinancials', label: 'View Page: Financials' },
            { key: 'viewEmergency', label: 'View Page: Emergency' },
            ...
            { key: 'viewModeration', label: 'View Page: Moderation' },
            { key: 'viewAuditLog', label: 'View Page: Audit Log' },
            { key: 'viewChat', label: 'View Page: Chat' },
            { key: 'viewPricing', label: 'View Page: Pricing' },
            { key: 'viewAccess', label: 'View Page: Access' },
            { key: 'viewSettings', label: 'View Page: Community Settings' },
            { key: 'viewLawEnforcement', label: 'View Page: Law Enforcement' },
            ...
            { key: 'viewPolls', label: 'View Page: Polls' },
    ...
    },
    {
        ...
        permissions: [
            ...
            { key: 'actionApproveGuestBook', label: 'Action: Approve Guest Book Reviews' },
            { key: 'actionDeleteContent', label: 'Action: Delete Content' },
            { key: 'canCreatePolls', label: 'Action: Create Polls' },
...
function PlatformSettingsContent() {
  ...
  const [liaisonOfficers, setLiaisonOfficers] = React.useState<TeamMember[]>([]);
  const [communityReportCategories, setCommunityReportCategories] = React.useState<any[]>([]);
  ...
  const [isAppointCourierDialogOpen, setIsAppointCourierDialogOpen] = React.useState(false);
  const [isRemoveCourierDialogOpen, setIsRemoveCourierDialogOpen] = React.useState(false);
  const [selectedCourierId, setSelectedCourierId] = React.useState<string>("");
  const [isUpdatingCourier, setIsUpdatingCourier] = React.useState(false);
  ...
  React.useEffect(() => {
    async function fetchOptions() {
        ...
        setCommunityReportCategories(options.ReportComLeader || []);
  ...
  React.useMemo(() => {
    if (allLiaisonOfficersData && communityData?.country && communityData?.state && communityData?.region) {
        setLiaisonOfficers(allLiaisonOfficersData.filter(officer => 
  ...
  React.useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
        ...
        const leadershipRoles = ['administrator', 'moderator', 'editor', 'reporter', 'broadcaster', 'vice-president', 'treasurer', 'secretary', 'leader', 'president', 'police-liaison-officer', 'community-courier'];
  ...
  const handlePoliceContactSave = async () => {
    if (!communityId) {
      toast({ title: "Error", description: "Community could not be identified.", variant: "destructive"});
      return;
    }
    if (!policeContact.officerId) {
         toast({ title: "Error", description: "An officer must be selected from the member list.", variant: "destructive"});
      return;
    }
    setIsSavingPoliceContact(true);
    
    const { officerName, contactEmail, ...restOfContact } = policeContact;

    const result = await runSavePoliceLiaison({ communityId, policeContact: restOfContact });
    if (result.success) {
        toast({ title: "Police Contact Saved" });
    } else {
        toast({ title: "Error", description: result.error, variant: "destructive" });
    }
  ...
  const handleAppointCourier = async () => {
    if (!communityId || !selectedCourierId) return;
    setIsUpdatingCourier(true);
    const result = await appointCommunityCourierAction({
        userId: selectedCourierId,
        communityId: communityId,
    });
    if (result.success) {
        toast({ title: 'Courier Appointed!', description: "The user can now set up their courier profile." });
        setIsAppointCourierDialogOpen(false);
    } else {
        toast({ title: 'Error', description: result.error, variant: 'destructive' });
    }
  ...
  const handleUnappointCourier = async () => {
    ...
    if (result.success) {
        toast({ title: 'Courier Unappointed', description: "The user has been removed from the courier role." });
        setIsRemoveCourierDialogOpen(false);
    } else {
        toast({ title: 'Error', description: result.error, variant: 'destructive' });
    }
    setIsUpdatingCourier(false);
  ...
  const handleAssignLiaison = async () => {
      if (!communityId || !selectedLiaisonId) {
          toast({ title: "Error", description: "No officer selected.", variant: 'destructive' });
          return;
      }
      setIsSavingPoliceContact(true);
  ...
  const impersonatingPermissions = (userProfile as any)?.impersonating?.permissions || {};
  const communityRoleDataForLoggedInUser = communityId ? userProfile?.communityRoles?.[communityId] : null;
  const activeRoleForLoggedInUser = communityRoleDataForLoggedInUser?.role || userProfile?.role;
  const loggedInUserIsPresident = activeRoleForLoggedInUser === 'president';
  const loggedInUserActivePermissions = { ...(userProfile?.permissions || {}), ...(communityRoleDataForLoggedInUser?.permissions || {}) };
  
  const canSetPermissions = ['owner', 'admin'].includes(activeRoleForLoggedInUser) || loggedInUserActivePermissions.actionSetTeamPermissions;

  const canSetPoliceLiaison = activeRoleForLoggedInUser === 'president' || loggedInUserActivePermissions.actionSetPoliceLiaison;
  const canSetCommunityBoundary = activeRoleForLoggedInUser === 'president' || loggedInUserActivePermissions.actionSetCommunityBoundary;
  ...
  return (
    <>
      ...
      <Accordion type="single" collapsible className="w-full">
        <AccordionItem value="team-roles" className="border rounded-lg">
            <AccordionTrigger className="p-6 hover:no-underline">
                <div className="flex flex-col text-left">
                    <CardTitle className="flex items-center gap-2"><UsersIcon /> Community Team Roles</CardTitle>
                    <CardDescription className="mt-1">Define and rank the roles within your leadership team. The order here represents seniority.</CardDescription>
                </div>
            </AccordionTrigger>
            <AccordionContent className="px-6 pb-6">
                 <div className="flex justify-end mb-4 pt-4 border-t">
                            ...
                            <div key={perm.key} className="md:col-span-3 lg:col-span-3">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3 mt-2 border p-4 rounded-md">
                                    {loadingCategories ? (
                                        ...
                                        communityReportCategories.map((option, index) => {
                                            const key = typeof option === 'object' ? (option as any).id || (option as any).name || index : option;
                                            const value = typeof option === 'object' ? (option as any).name || option : option;
                                            return (
                                                <div key={key} className="flex items-center space-x-2">
                                                    <Checkbox
                                                        id={`cat-${key}`}
                                                        checked={(permissions.viewableReportCategories as string[])?.includes(value)}
                                                        onCheckedChange={(checked) => handleCategoryPermissionChange(value, !!checked)}
                                                        disabled={isPresidentSelected || !canSetPermissions}
                                                    />
                                                    <Label htmlFor={`cat-${key}`} className="font-normal text-sm">
                                                        {value}
                                                    </Label>
                                                </div>
                                            )
                        ...
                        <AlertDialogContent>
                            <AlertDialogHeader>
                                ...
                                <AlertDialogDescription>
                                    This will unassign {allMembers.find(m => m.id === policeContact.officerId)?.name} as the police liaison for your community. They will no longer receive relevant reports.
                                </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
       ...
       <fieldset>
        <Card>
            <CardHeader>
                ...
                <CardDescription>Designate a single, official courier for your community's storefront deliveries.</CardDescription>
            </CardHeader>
            <CardContent>
                {communityData?.courierId ? (
                    <div className="p-4 border rounded-lg bg-muted/50">
                        <p><strong>Current Courier:</strong> {allMembers.find(m => m.id === communityData.courierId)?.name || 'Unknown'}</p>
                    </div>
                ) : (
                    <p className="text-sm text-muted-foreground">No courier has been appointed for this community yet.</p>
                )}
            </CardContent>
             <CardFooter className="gap-2">
                <Dialog open={isAppointCourierDialogOpen} onOpenChange={setIsAppointCourierDialogOpen}>
                    <DialogTrigger asChild>
                         <Button variant="outline" disabled={!!communityData?.courierId}>
                            <UserPlus className="mr-2 h-4 w-4" />
                            {communityData?.courierId ? 'Courier Appointed' : 'Appoint Courier'}
                        </Button>
                    </DialogTrigger>
                    <DialogContent>
                         <DialogHeader>
                            <DialogTitle>Appoint Community Courier</DialogTitle>
                            <DialogDescription>
                                Select a member to be the official delivery provider for your community storefront.
                            </DialogDescription>
                        </DialogHeader>
                        <div className="py-4 space-y-2">
                            <Label>Select an approved applicant:</Label>
                            <Select value={selectedCourierId} onValueChange={setSelectedCourierId}>
                                <SelectTrigger>
                                    <SelectValue placeholder="Select a member..." />
                                </SelectTrigger>
                                <SelectContent>
                                    {allMembers.filter(m => m.permissions?.isCourier).map(member => (
                        ...
                        <DialogFooter>
                            <Button variant="outline" onClick={() => setIsAppointCourierDialogOpen(false)}>Cancel</Button>
                            <Button onClick={handleAppointCourier} disabled={!selectedCourierId || isUpdatingCourier}>
                                {isUpdatingCourier && <Loader2 className="mr-2 h-4 w-4 animate-spin"/>}
                                Confirm Appointment
                            </Button>
                        </DialogFooter>
                    </DialogContent>
                ...
                <AlertDialog open={isRemoveCourierDialogOpen} onOpenChange={setIsRemoveCourierDialogOpen}>
                    <AlertDialogTrigger asChild>
                        <Button variant="destructive" disabled={!communityData?.courierId}>
                     ...
                     <AlertDialogContent>
                        <AlertDialogHeader>
                            <AlertDialogTitle>Are you sure?</AlertDialogTitle>
                            <AlertDialogDescription>
                                This will remove the current courier. Storefront deliveries will be disabled until a new courier is appointed.
                            </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
                            <AlertDialogCancel>Cancel</AlertDialogCancel>
                            <AlertDialogAction onClick={handleUnappointCourier} disabled={isUpdatingCourier}>
                                {isUpdatingCourier && <Loader2 className="mr-2 h-4 w-4 animate-spin"/>}
                                Confirm Unappointment
                            </AlertDialogAction>
                        </AlertDialogFooter>
                    </AlertDialogContent>
                </AlertDialog>
      ...
      <fieldset disabled={!canSetCommunityBoundary}>
        <Card className={cn(!canSetCommunityBoundary && "opacity-60")}>
            <CardHeader>
                <CardTitle className="flex items-center gap-2"><MapPinned className="h-5 w-5" />Community Boundary</CardTitle>
