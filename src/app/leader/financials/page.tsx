...
export default function FinancialsPage() {
  ...
  const [listingShare, setListingShare] = React.useState(0);
  const [storefrontShare, setStorefrontShare] = React.useState(0);
  ...
  const [storefrontSubscriptionCount, setStorefrontSubscriptionCount] = React.useState(0);
  ...
  const isStripeConnected = communityData?.stripeAccountId && communityData?.stripeAccountOwnerId === user?.uid;
  const isHandoverScenario = communityData?.stripeAccountId && communityData?.stripeAccountOwnerId !== user?.uid;
  ...
  React.useEffect(() => {
    if (!communityId || !db || !user || !businessPlan || !enterprisePlan || !communityData || !storefrontPlan) {
        setLoading(false);
        return;
    }
    ...
    const businessesUnsubscribe = onSnapshot(businessesQuery, (businessesSnapshot) => {
        ...
        let totalListingRevenue = 0;
        let totalStorefrontRevenue = 0;
        ...
        businessesSnapshot.docs.forEach(doc => {
            ...
            if (data.status === 'Subscribed') {
                if (data.isFreeListing) {
                    ...
                    totalListingRevenue += price;
                }
            }

            // Storefront subscription revenue
            if (data.storefrontSubscription === true) {
                storefrontCount++;
                totalStorefrontRevenue += storefrontPlan.monthlyPrice;
            }
        });

        const netListingIncome = totalListingRevenue * (1 - stripeFeePercentage);
        const netStorefrontIncome = totalStorefrontRevenue * (1 - stripeFeePercentage);
        const currentTotalNetIncome = netListingIncome + netStorefrontIncome;

        const revenueSharePercentage = (communityData?.revenueShare ?? 40) / 100;
        const currentListingShare = netListingIncome * revenueSharePercentage;
        const currentStorefrontShare = netStorefrontIncome * 0.10; // 10% for storefronts
        const totalCommunityShare = currentListingShare + currentStorefrontShare;
        
        setBusinessSubscriptionCount(paidBizCount);
        setEnterpriseSubscriptionCount(paidEntCount);
        setStorefrontSubscriptionCount(storefrontCount);
        setFreeListingCount(freeCount);

        setTotalMonthlyIncome(currentTotalNetIncome);
        setCommunityShare(totalCommunityShare);
        setListingShare(currentListingShare);
        setStorefrontShare(currentStorefrontShare);
  ...
  }, [userProfile, businessPlan, enterprisePlan, storefrontPlan, communityId, db, user, communityData, eligibility, isStripeConnected]);
  ...
  const handleConnectStripe = async () => {
    if (!communityId || !user) {
        toast({ title: "Error", description: "Community ID or User ID not found.", variant: "destructive" });
        return;
    }
    setIsRedirecting(true);
    const result = await createStripeConnectAccountLinkForCommunity({ communityId, userId: user.uid });
    if (result.url) {
  ...
  const activeRole = impersonating?.role || userProfile?.role;
  const permissions = impersonating?.permissions || userProfile?.permissions || {};
  const canViewFinancials = ['president', 'owner', 'admin'].includes(activeRole) || permissions.viewFinancials;
  
  const finalLoadingState = isUserLoading || profileLoading || communityLoading || loadingEligibility || loading;
    ...
    return (
        <div className="space-y-8">
            <Alert variant="destructive">
                ...
                <AlertDescription>
                    You do not have the required permissions to view this page. Please contact your community leader or an administrator.
                </AlertDescription>
            </Alert>
        </div>
    );
             ...
             <Card>
                ...
                <CardContent>
                    ...
                    <p className="text-xs text-muted-foreground">
                        {businessSubscriptionCount} Business, {enterpriseSubscriptionCount} Enterprise, & {storefrontSubscriptionCount} Storefront subscribers
             ...
             <Card>
                ...
                <CardContent>
                    ...
                    <div className="text-xs text-muted-foreground space-y-1">
                        <div className="flex justify-between"><span>From Listings ({currentRevenueShare ?? '40'}%):</span> <span>{new Intl.NumberFormat("en-GB", { style: "currency", currency: "GBP" }).format(listingShare)}</span></div>
                        <div className="flex justify-between"><span>From Storefronts (10%):</span> <span>{new Intl.NumberFormat("en-GB", { style: "currency", currency: "GBP" }).format(storefrontShare)}</span></div>
                    </div>
                </CardContent>
                            ...
                            <RequirementItem step={1} title="Accept Financial Terms" isMet={eligibility.termsAccepted}>
                                {eligibility.termsAccepted ? (
                                    ...
                                    <Dialog open={isTermsDialogOpen} onOpenChange={setIsTermsDialogOpen}>
                                        ...
                                        <DialogContent className="max-w-3xl grid-rows-[auto,1fr,auto] p-0 max-h-[90vh]">
                             ...
                             <RequirementItem step={2} title="Community Status: Active" isMet={eligibility.isProfileComplete}>
                                {
                                    eligibility.isProfileComplete 
                                        ...
                                        : "Your community must be active to earn revenue. This status is set by platform admins after review."
                                }
                            </RequirementItem>
                             <RequirementItem step={3} title="Connect Stripe Account" isMet={isStripeConnected}>
                                {isStripeConnected ? (
                                    ...
                                    <>
                                        <p>A new leader has been assigned. You must connect your own Stripe account to receive future payouts.</p>
                                        <Alert variant="destructive" className="mt-2">
                                            <AlertTriangle className="h-4 w-4" />
                                            <AlertTitle>Action Required!</AlertTitle>
                                            <AlertDescription>
                                                Until you connect a new account, all community revenue will continue to be sent to the previous leader's account. If the previous account was not a general community bank account, this requires your immediate attention to ensure funds are correctly routed.
                                            </AlertDescription>
                                        </Alert>
                                    </>
                                ) : (
            ...
            <CardHeader>
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                        ...
                        <CardDescription>A record of all revenue share payouts to be made to you.</CardDescription>
                    </div>
                </div>
            </CardHeader>
            <CardContent>
                <Alert>
                    ...
                    <AlertTitle>View Your Payout History on Stripe</AlertTitle>
                    <AlertDescription>
