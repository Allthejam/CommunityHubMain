...
export default function FinancialsPage() {
  ...
  React.useEffect(() => {
    ...
    const accepted = !!userProfile?.termsAcceptedAt;
  ...
  const handleAcceptTerms = async () => {
    if (!user) return;
    setIsAccepting(true);
    const result = await acceptTermsAction(user.uid);
    if (result.success) {
      toast({
  ...
  return (
    <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
             ...
             <Card>
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    <CardTitle className="text-sm font-medium">Your Community Revenue Share ({currentRevenueShare ?? 'N/A'}%)</CardTitle>
        ...
        <Card>
            ...
            <CardContent>
                <div className="rounded-md border">
                    <Table>
                        <TableHeader>
                            <TableRow>
                                ...
                                <TableHead><Button variant="ghost" onClick={() => handleSort('amount')}>Amount <ArrowUpDown className="ml-2 h-4 w-4" /></Button></TableHead>
                        ...
                        {loading ? (
                            ...
                            paginatedPayouts.map((payout) => (
                                <TableRow key={payout.transactionId}>
                                    ...
                                    <TableCell>{payout.community}</TableCell>
                                    <TableCell className="font-medium">
                                        {new Intl.NumberFormat("en-GB", { style: "currency", currency: "GBP" }).format(payout.amount)}
                                    </TableCell>
                 ...
                 <PaginationControls pagination={pagination} setPagination={setPagination} pageCount={pageCount} totalRows={filteredPayouts.length} />
            </CardContent>
        </Card>
    </div>
  )
