...
const initialDayState = { open: '', close: '', closed: false };
    ...
    React.useEffect(() => {
        if (itemData && itemCategories.length > 0) {
            ...
            if (itemData.openingHours && Object.keys(itemData.openingHours).length > 0) {
                const completeHours = {
                    monday: { ...initialHoursState.monday, ...(itemData.openingHours.monday || {}) },
                    tuesday: { ...initialHoursState.tuesday, ...(itemData.openingHours.tuesday || {}) },
                    wednesday: { ...initialHoursState.wednesday, ...(itemData.openingHours.wednesday || {}) },
                    thursday: { ...initialHoursState.thursday, ...(itemData.openingHours.thursday || {}) },
                    friday: { ...initialHoursState.friday, ...(itemData.openingHours.friday || {}) },
                    saturday: { ...initialHoursState.saturday, ...(itemData.openingHours.saturday || {}) },
                    sunday: { ...initialHoursState.sunday, ...(itemData.openingHours.sunday || {}) },
                };
                setOpeningHours(completeHours);
                setShowOpeningHours(true);
            } else {
                setOpeningHours(initialHoursState);
                    ...
                    <CardContent className="space-y-6">
                        ...
                        <Separator />
                         <div className="space-y-4">
                            ...
                            {showOpeningHours && (
                            <div className="space-y-4">
                                <div className="hidden md:grid grid-cols-[100px_1fr_1fr_auto] items-center gap-x-4 text-sm font-medium text-muted-foreground px-2">
                                    <span></span>
                                    <span>Open</span>
                                    <span>Close</span>
                                </div>
                                {Object.keys(openingHours).map(day => (
                                    <div key={day} className="grid grid-cols-1 md:grid-cols-[100px_1fr_1fr_auto] items-center gap-x-4 gap-y-2 p-2 rounded-md hover:bg-muted/50">
