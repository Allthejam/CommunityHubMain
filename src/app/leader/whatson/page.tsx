...
import { updateWhatsonStatusAction, deleteWhatsonItemAction } from "@/lib/actions/whatsonActions";
...
export default function LeaderWhatsonPage() {
    ...
    const handleSort = (key: keyof WhatsonItem) => {
        setSorting(prev => ({
            key,
    ...
    const sortedItems = React.useMemo(() => {
        let filtered = whatsonItems;
        if (activeTab !== 'all') {
            filtered = whatsonItems.filter(item => item.status === activeTab);
        }
    ...
    }, [whatsonItems, activeTab, sorting]);
    ...
    const pageCount = Math.ceil(sortedItems.length / pagination.pageSize);
        ...
        <CardContent>
          <div className="rounded-md border">
            <Table>
              <TableHeader>
                <TableRow>
                    <TableHead><Button variant="ghost" onClick={() => handleSort('title')}>Item / Venue <ArrowUpDown className="ml-2 h-4 w-4" /></Button></TableHead>
            ...
            <PaginationControls pagination={pagination} setPagination={setPagination} pageCount={pageCount} totalRows={sortedItems.length} />
        </CardContent>
      </Card>
    </div>
  );
