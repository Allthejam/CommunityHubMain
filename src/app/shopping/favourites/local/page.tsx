...
export default function LocalFavouritesPage() {
    ...
    const localFavourites = React.useMemo(() => {
        if (!favouriteBusinesses || !userProfile?.communityId) return [];
        return favouriteBusinesses.filter(b => b.primaryCommunityId === userProfile.communityId);
    }, [favouriteBusinesses, userProfile?.communityId]);
