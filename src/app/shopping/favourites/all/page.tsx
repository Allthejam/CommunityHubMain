...
export default function AllFavouritesPage() {
    ...
    const activeFavourites = React.useMemo(() => {
        if (!favouriteBusinesses) return [];
        return favouriteBusinesses.filter(
            (b) => b.storefrontSubscription && (b.status === 'Subscribed' || b.status === 'Approved')
        );
    }, [favouriteBusinesses]);
    ...
    return (
        <div className="space-y-8">
            ...
            ) : activeFavourites && activeFavourites.length > 0 ? (
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6">
                    {activeFavourites.map(business => (
