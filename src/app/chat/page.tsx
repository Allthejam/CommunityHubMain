...
type Conversation = {
    ...
    otherMember?: TeamMember; // Used for provisional chats
};
...
function ChatPageContent() {
    ...
    React.useEffect(() => {
     ...
     React.useEffect(() => {
        if (!currentChatId || !user || !db || currentChatId.startsWith('temp_')) {
            setMessages([]);
    ...
    const handleSendMessage = async (e: React.FormEvent) => {
        e.preventDefault();
        if ((!newMessage.trim() && !imageToSend) || !user || !userProfile || !db) return;

        if (provisionalConversation && currentChatId === provisionalConversation.id) {
            setIsSubmitting(true);
            try {
                const memberIds = [user.uid, provisionalConversation.otherMember!.id];
                const newGroupName = `${userProfile.name} & ${provisionalConversation.otherMember!.name}`;
                ...
                setProvisionalConversation(null);
                setNewMessage('');
                setImageToSend(null);
    ...
    const handleSelectUser = async (member: TeamMember) => {
        ...
        if (existingChat) {
            ...
            // Create a temporary conversation object without saving it to Firestore
            const tempChatId = `temp_${member.id}`;
            const tempConversation: Conversation = {
                id: tempChatId,
                name: `${userProfile.name} & ${member.name}`,
                avatar: member.avatar || '',
                lastMessage: `Start a conversation with ${member.name}`,
                isArchived: false,
                lastMessageTimestamp: null,
                createdBy: user.uid,
                scope: 'private',
                memberIds: [user.uid, member.id],
                otherMember: member,
                communityId: communityId!,
                isPlatformChat: false,
            };
            
            setProvisionalConversation(tempConversation);
    ...
    const selectedConversation = conversations.find(c => c.id === currentChatId) || (provisionalConversation && provisionalConversation.id === currentChatId ? provisionalConversation : null);

    
    const otherMemberId = React.useMemo(() => {
        if (!selectedConversation || !user) return null;
        if (selectedConversation.id.startsWith('temp_')) {
            return selectedConversation.otherMember?.id;
        }
        return selectedConversation.memberIds.find(id => id !== user.uid);
    }, [selectedConversation, user]);

    const otherMemberProfileRef = useMemoFirebase(() => {
        if (!otherMemberId || !db) return null;
        return doc(db, 'users', otherMemberId);
    }, [otherMemberId, db]);
    const { data: otherMemberProfile } = useDoc<User>(otherMemberProfileRef);
                        ...
                        <>
                            <header className="h-20 bg-white border-b border-slate-200 px-4 md:px-6 flex items-center justify-between flex-shrink-0 relative z-20 shadow-sm">
                                <div className="flex items-center space-y-1 min-w-0">
                                    <Button onClick={() => { setCurrentChatId(null); setProvisionalConversation(null); }} variant="ghost" className="md:hidden mr-3 p-2 text-slate-500 hover:bg-slate-100 rounded-full transition-colors">
                                        <ArrowLeft />
                                    </Button>
                                    ...
                                    <div className="min-w-0">
                                        ...
                                        {otherMemberProfile ? (
                                            <p id="currentChatStatus" className="text-xs flex items-center">
                                                <span className={cn("w-2 h-2 rounded-full mr-1.5", otherMemberProfile.isOnline ? "bg-green-500 animate-pulse" : "bg-gray-400")}></span>
                                                <span className={cn(otherMemberProfile.isOnline ? "text-green-600" : "text-slate-400")}>
                                                    {otherMemberProfile.isOnline ? 'Online' : (otherMemberProfile.lastSeen ? `Last seen ${formatDistanceToNow(otherMemberProfile.lastSeen.toDate(), { addSuffix: true })}` : 'Offline')}
                                                </span>
                                            </p>
                                        ) : (
