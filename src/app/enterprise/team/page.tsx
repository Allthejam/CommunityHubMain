...
export default function EnterpriseTeamPage() {
  ...
  const enterpriseQuery = useMemoFirebase(() => {
    if (!user) return null;
    return query(
  ...
  const { data: businesses, isLoading: businessesLoading } = useCollection<Business>(enterpriseQuery);
  ...
  const handleAddMember = async () => {
    if (!selectedBusinessId || !searchResult || (searchResult as any).error) return;
    ...
    if (result.success) {
        toast({ title: "Member Added", description: `${searchResult.name || 'The user'} has been added to the group team.` });
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-muted-foreground">
          Invite and manage team members for your specific enterprise groups.
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Select an enterprise group to manage its team.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          <div className="max-w-sm">
             <Select onValueChange={setSelectedBusinessId} disabled={loading || !businesses || businesses.length === 0} value={selectedBusinessId || ''}>
                <SelectTrigger>
                    <SelectValue placeholder="Select an enterprise group..." />
                </SelectTrigger>
                <SelectContent>
                    {businesses?.map(biz => (
                  ...
                  {selectedBusiness?.team && selectedBusiness.team.length > 0 ? (
                    selectedBusiness.team.map((member) => (
                      <ContextMenu key={member.userId}>
                        <ContextMenuTrigger asChild>
                            <TableRow>
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                    <p className="font-medium text-sm">{member.name || 'Anonymous'}</p>
                                ...
                                <DropdownMenu>
                                    ...
                                    <DropdownMenuContent align="end">
                                        ...
                                        <DropdownMenuItem onClick={() => openEditDialog(member)}><FileEdit className="mr-2 h-4 w-4" /> Edit Role</DropdownMenuItem>
                                        <DropdownMenuSeparator />
                        ...
                        <ContextMenuContent>
                            ...
                            <ContextMenuItem onSelect={() => openEditDialog(member)}><FileEdit className="mr-2 h-4 w-4" /> Edit Role</ContextMenuItem>
                            <ContextMenuSeparator />
                            <ContextMenuItem className="text-destructive focus:text-destructive" onSelect={() => handleRemoveMember(member.userId)}><Trash2 className="mr-2 h-4 w-4" /> Remove from Group</ContextMenuItem>
                        </ContextMenuContent>
                      </ContextMenu>
                    ))
                  ) : (
                    <TableRow>
                      <TableCell colSpan={3} className="h-24 text-center">
                        No team members added to this group yet.
                      </TableCell>
                    </TableRow>
                  )}
                </TableBody>
        ...
        <CardFooter>
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                ...
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>Invite to {selectedBusiness?.businessName}</DialogTitle>
                        <DialogDescription>Search for an existing community user by email to add them to this specific group.</DialogDescription>
                    </DialogHeader>
                    <div className="py-4 space-y-4">
                        ...
                        <Card>
                            ...
                            {(searchResult as any).error ? (
                                <p className="text-sm text-destructive">{(searchResult as any).error}</p>
                            ) : (
                                ...
                                <div className="space-y-2">
                                    <div className="flex items-center gap-2">
                                        <Label htmlFor="role-select">Assign Enterprise Role</Label>
                                        <Dialog>
                                            <DialogTrigger asChild>
                                            ...
                                            <DialogContent>
                                                <DialogHeader>
                                                    <DialogTitle>Enterprise Role Permissions</DialogTitle>
                                                </DialogHeader>
                                                <div className="py-4 space-y-4">
                                                    <div>
                                                        <h4 className="font-semibold">Content Editor</h4>
                                                        <p className="text-sm text-muted-foreground">Manage profile content, galleries, and events for this specific group.</p>
                                                    </div>
                                                    <div>
                                                        <h4 className="font-semibold">Advert Manager</h4>
                                                        <p className="text-sm text-muted-foreground">Create and manage adverts and promotions linked to this group.</p>
                                                    </div>
                                                    <div>
                                                        <h4 className="font-semibold">Manager</h4>
                                                        <p className="text-sm text-muted-foreground">Full oversight of group content and team members (excluding billing).</p>
                                                    </div>
                                                </div>
                                            </DialogContent>
                                        </Dialog>
                    ...
                    <DialogFooter>
                        ...
                        <Button onClick={handleAddMember} disabled={isAdding || !searchResult || !!(searchResult as any).error}>
            ...
            <Dialog open={!!memberToEdit} onOpenChange={() => setMemberToEdit(null)}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>Update the role for this team member within {selectedBusiness?.businessName}.</DialogDescription>
                    </DialogHeader>
                    <div className="py-4">
