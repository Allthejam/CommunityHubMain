...
import { Building2, ArrowLeft, Info, PlusCircle, Trash2, Loader2, Save, Eye, ShieldAlert, Search, ImagePlus, X, Upload, Pencil, UserCheck, Mail, Phone, MapPin, Clock } from "lucide-react";
...
export default function EditEnterpriseGroupPage() {
    ...
    React.useEffect(() => {
        if (groupData) {
            ...
            setIsAccommodation(groupData.isAccommodation || false);
            ...
            setPageThreeType(groupData.pageThreeType || (groupData.keyContacts?.length || groupData.meetingLocation ? 'contact' : (groupData.meetingMinutes?.length ? 'minutes' : 'contact')));
            setMeetingMinutes(groupData.meetingMinutes || []);
            setKeyContacts(groupData.keyContacts || []);
            setMeetingLocation(groupData.meetingLocation || {
                venueName: '',
                addressLine1: '',
                city: '',
                postcode: '',
            ...
            setEnableContactForm(groupData.enableContactForm !== false);
            setContactIntroText(groupData.contactIntroText || '');
    ...
    const handleSave = async (status: 'Pending Approval' | 'Draft') => {
        ...
        try {
            const result = await runCreateBusiness({
                businessId: groupId,
                businessName, businessCategory, isAccommodation, website, socialMedia, contactEmail, contactNumber,
            ...
            <TabsContent value="page1">
                <CardContent className="space-y-6">
                    ...
                    <div className="grid md:grid-cols-2 gap-6">
                        ...
                        <div className="grid gap-2">
                            <Label htmlFor="category">{isAccommodation ? 'Accommodation Type *' : 'Category (Optional)'}</Label>
             ...
             <TabsContent value="page3">
                <CardContent className="space-y-6">
                    ...
                    <div className="space-y-3 p-4 bg-muted/30 rounded-lg border">
                        ...
                        >
                            <div className={cn("flex items-start space-x-2 border p-3 rounded-md cursor-pointer transition-colors", pageThreeType === 'contact' ? 'bg-primary/5 border-primary' : 'bg-card')}>
                                <RadioGroupItem value="contact" id="p3-type-contact" className="mt-1" />
                                <div className="space-y-1">
                                    <Label htmlFor="p3-type-contact" className="font-semibold cursor-pointer text-sm">Contact Directory</Label>
                            ...
                            <div className={cn("flex items-start space-x-2 border p-3 rounded-md cursor-pointer transition-colors", pageThreeType === 'minutes' ? 'bg-primary/5 border-primary' : 'bg-card')}>
                                <RadioGroupItem value="minutes" id="p3-type-minutes" className="mt-1" />
                                <div className="space-y-1">
                                    <Label htmlFor="p3-type-minutes" className="font-semibold cursor-pointer text-sm">Meeting Minutes</Label>
                            ...
                            <div className={cn("flex items-start space-x-2 border p-3 rounded-md cursor-pointer transition-colors", pageThreeType === 'custom' ? 'bg-primary/5 border-primary' : 'bg-card')}>
                                <RadioGroupItem value="custom" id="p3-type-custom" className="mt-1" />
                                <div className="space-y-1">
                                    <Label htmlFor="p3-type-custom" className="font-semibold cursor-pointer text-sm">Custom Document</Label>
                    ...
                    {pageThreeType === 'minutes' && (
                        <div className="space-y-4">
                            <Alert>
                                ...
                                <AlertDescription>
                                    Your group&apos;s meeting minutes and PDF records are managed under the Meeting Minutes section.
                                </AlertDescription>
                            </Alert>
                        </div>
                    )}
                    ...
                    {pageThreeType === 'custom' && (
                        <div className="space-y-2">
                            <Label htmlFor="page-three-content">Custom Content / Legal Terms</Label>
                            <RichTextEditor
                                value={pageThreeContent}
                                onChange={setPageThreeContent}
                                placeholder="Enter custom text, regulatory policies, or terms..."
                            />
                        </div>
                    )}
                </CardContent>
