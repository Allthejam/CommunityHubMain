...
export default function EnterpriseGalleryPage() {
  ...
  const galleryQuery = useMemoFirebase(
    () => (selectedBusinessId ? query(collection(db, `businesses/${selectedBusinessId}/gallery`), orderBy('createdAt', 'desc')) : null),
  ...
  const loading = authLoading || businessesLoading || imagesLoading;
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
        ...
        const uploadResult = await uploadImageAction({ base64Data, path: storagePath });
        if (!uploadResult.success || !uploadResult.url) {
            throw new Error(uploadResult.error || "Failed to get image URL.");
        }
