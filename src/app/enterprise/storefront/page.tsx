    ...
    return (
        <Card>
            <CardHeader>
                ...
                <CardDescription>Set your group's weekly opening hours.</CardDescription>
            </CardHeader>
                        ...
                        >
                            <div className="flex items-start space-x-3 p-3 border rounded-lg cursor-pointer hover:bg-muted/30">
                                ...
                                <Label htmlFor="cc" className="flex-1 cursor-pointer">
                                    ...
                                    <p className="text-xs text-muted-foreground mt-1">Customers will collect their orders from your group location.</p>
                                </Label>
                            </div>
                            ...
                            <div className={cn(
                                ...
                                <Label htmlFor="sd" className={cn("flex-1", canAcceptPayments ? "cursor-pointer" : "cursor-default")}>
                                    <span className="font-bold">Group Delivery / Free Delivery</span>
                                    <p className="text-xs text-muted-foreground mt-1">The enterprise group will arrange and handle local deliveries themselves.</p>
                                </Label>
                            </div>
                            ...
                            <div className={cn(
                                ...
                                <Label htmlFor="lc" className={cn("flex-1", (isCourierReady && canAcceptPayments) ? "cursor-pointer" : "cursor-default")}>
                                    ...
                                    <p className="text-xs text-muted-foreground mt-1">
                                        Handled by the official community courier. A delivery fee will be added at checkout.
                                    </p>
                                </Label>
                            </div>
                        </RadioGroup>
...
export default function EnterpriseStorefrontPage() {
  ...
  const enterpriseGroupsQuery = useMemoFirebase(() => {
    if (!user) return null;
    return query(
  ...
  const { data: businesses, isLoading: businessesLoading } = useCollection<Business>(enterpriseGroupsQuery);
  ...
  const handleConnectStripe = async () => {
    ...
    const result = await createStripeConnectAccountLinkForBusiness(selectedBusinessId, '/enterprise/storefront');
  ...
  if (!businessesLoading && storefrontBusinesses.length === 0) {
    return (
        <div className="flex flex-col items-center justify-center min-h-[400px] text-center space-y-4">
            ...
            <h2 className="text-2xl font-bold font-headline">Enterprise Storefronts</h2>
            <p className="text-muted-foreground max-w-md">
                You haven't opened a storefront for any of your enterprise groups yet. A storefront subscription is required for each group to manage products and orders.
            </p>
            <div className="flex gap-4">
                <Button asChild variant="outline">
                    <NextLink href="/enterprise/groups">Manage My Groups</NextLink>
                </Button>
            </div>
        </div>
    );
    ...
    <div className="space-y-8">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
                ...
                <p className="text-muted-foreground">Manage your group products, orders, and fulfillment settings.</p>
            </div>
        ...
        {selectedBusiness?.sellsRestrictedProducts && (
            <Alert variant="destructive" className="bg-red-50 border-red-200 text-red-900 dark:bg-red-900/20 dark:border-red-800 dark:text-red-200">
                ...
                <AlertDescription className="text-sm font-medium">
                    This group is marked as selling restricted products (Alcohol/Tobacco). Ensure all age verification procedures are followed. Online sales are age-gated to 21+.
                </AlertDescription>
            </Alert>
        )}
                    ...
                    <CardContent>
                        ...
                        products && products.length > 0 ? (
                            ...
                            <div className="text-center py-8 border-2 border-dashed rounded-lg">
                                ...
                                <p className="text-sm text-muted-foreground">No products added for this group yet.</p>
                                <Button asChild variant="link" className="mt-2">
                                    <NextLink href={`/business/products/create?businessId=${selectedBusinessId}`}>Add group product</NextLink>
                                </Button>
                            </div>
                        )}
                    </CardContent>
                        ...
                        <Table>
                            <TableHeader>
                             ...
                             <TableBody>
                                ...
                                ) : <TableRow><TableCell colSpan={3} className="text-center py-6">No recent orders for this group.</TableCell></TableRow>}
                            </TableBody>
                        </Table>
                    </CardContent>
                    <CardFooter>
                        <Button asChild variant="outline" className="w-full">
                           <NextLink href="/business/orders">View All Group Orders</NextLink>
                        </Button>
                    </CardFooter>
                </Card>
            ...
            <div className="space-y-8">
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Each group requires its own Stripe account for payouts.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        {canAcceptPayments ? (
                            ...
                            <Alert variant="default" className="bg-amber-50 border-amber-200">
                                <Info className="h-4 w-4 text-amber-600" />
                                <AlertTitle className="text-amber-800">Payout Setup Required</AlertTitle>
                                <AlertDescription>
                                    Connect Stripe for this specific group to enable online checkout.
                                </AlertDescription>
                            </Alert>
                        )}
                    </CardContent>
                    <CardFooter>
                        <Button onClick={handleConnectStripe} disabled={isRedirecting || !selectedBusinessId} className="w-full">
                            ...
                            {canAcceptPayments ? 'Manage Stripe Connection' : 'Connect Group to Stripe'}
                        </Button>
                    </CardFooter>
                </Card>
            </div>
