...
const NotificationRow = React.memo(({ notification, onUpdateStatus, onDelete, onViewDetails, isSelected, onSelect }: { 
...
export default function NotificationsPage() {
    ...
    const handleViewDetails = React.useCallback(async (notification: Notification) => {
        ...
        if (notification.details?.declineReason) {
            setDeclineReason(notification.details.declineReason);
        } else {
            setDeclineReason("A specific reason was not attached to this notification.");
        }
    }, []);
    ...
    <>
        ...
        <Dialog open={!!viewingNotification} onOpenChange={() => setViewingNotification(null)}>
            <DialogContent>
                <DialogHeader>
                    <DialogTitle>{viewingNotification?.subject}</DialogTitle>
                    <DialogDescription>
                        The administration team has provided the following reason for this decision.
                    </DialogDescription>
                </DialogHeader>
                <div className="py-4">
                    <p className="text-sm p-4 bg-muted rounded-md">{declineReason}</p>
                </div>
                <DialogFooter>
