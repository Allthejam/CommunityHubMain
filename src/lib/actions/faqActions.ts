...
import { generateFaqDraft, type GenerateFaqDraftOutput } from '@/ai/flows/generate-faq-draft';

export async function aiGenerateFaqDraftAction({ rawText }: { rawText: string }): Promise<{
  success: boolean;
  error?: string;
  data?: GenerateFaqDraftOutput;
}> {
  console.log('AI generating FAQ draft...');
  try {
    const data = await generateFaqDraft({ rawText });
    return { success: true, data };
  } catch (error: any) {
    console.error('Error in aiGenerateFaqDraftAction:', error);
    return { success: false, error: error.message };
  }
}

export async function runBulkCreateFaqs(params: {
  communityId: string;
  items: Array<{ question: string; answer: string }>;
}): Promise<ActionResponse> {
  const { communityId, items } = params;
  if (!communityId || !items || items.length === 0) {
    return { success: false, error: 'Community ID and at least one FAQ item are required.' };
  }
  try {
    const { firestore } = initializeAdminApp();
    const faqsCollection = firestore.collection(`communities/${communityId}/faqs`);
    ...
    // Get the current max order
    const snapshot = await faqsCollection.orderBy('order', 'desc').limit(1).get();
    let currentOrder = snapshot.empty ? 0 : snapshot.docs[0].data().order;

    const batch = firestore.batch();

    for (const item of items) {
      currentOrder += 1;
      const newRef = faqsCollection.doc();
      batch.set(newRef, {
    ...
    console.error('Error in runBulkCreateFaqs:', error);
    return { success: false, error: error.message };
  }
}
