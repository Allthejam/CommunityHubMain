...
export async function updateNotificationStatusAction(params: {
  ...
  try {
    ...
    // If marking as read, check if we need to enrich it with a decline reason for older notifications
    if (status === 'read') {
        try {
            ...
            if (notificationDoc.exists()) {
                ...
                if (
                    data.type === 'Special Access Request' && 
                    data.subject.toLowerCase().includes('declined') && 
                    !data.details?.declineReason && 
                    data.relatedId
                ) {
                    const requestRef = firestore.collection('access_requests').doc(data.relatedId);
                    const requestDoc = await requestRef.get();
                    if (requestDoc.exists() && requestDoc.data()?.declineReason) {
                        updateData['details.declineReason'] = requestDoc.data()!.declineReason;
                    }
                }
            }
        } catch (enrichError: any) {
            // Log the enrichment error but don't fail the entire status update.
            // The main goal is to mark the notification as read.
            console.error(`[Enrichment Error] Failed to get decline reason for notification ${notificationId}: ${enrichError.message}`);
        }
    }
