...
export function ValuedPartners({ layout = 'fade' }: { layout?: 'fade' | 'carousel' }) {
    ...
    const partnersQuery = useMemoFirebase(() => {
        ...
        return query(
            ...
            where('status', 'in', ['Active', 'Scheduled']), // Fetch both
            where('type', '==', 'partner') // Only partner ads
        );
    }, [db]);
    ...
    React.useEffect(() => {
        ...
        const sourceAds = (activeAdverts && activeAdverts.length > 0) ? activeAdverts : mockPartners.map(p => ({
            id: p.id,
            headline: p.name,
        ...
        if (user && userProfile && userProfile.settings?.adPersonalization) {
            ...
            if (userCategories.length === 0) {
                 eligibleAds = validDateAds;
            } else {
                 eligibleAds = validDateAds.filter(ad => {
        ...
        if (eligibleAds.length === 0 && validDateAds.length > 0) {
            eligibleAds = validDateAds;
        }
    ...
    }, [activeAdverts, user, userProfile, partnersLoading, profileLoading]);
