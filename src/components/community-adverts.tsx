  ...
  React.useEffect(() => {
    ...
    const fetchOnSaleProducts = async () => {
      ...
      businesses.forEach(businessDoc => {
        ...
        if (businessDoc.status === 'Subscribed') {
          isEligible = true;
        } else if (businessDoc.status === 'Approved' && businessDoc.createdAt) {
          const creationDate = businessDoc.createdAt.toDate();
          const daysSinceCreation = differenceInDays(now, creationDate);
          if (daysSinceCreation <= 14) isEligible = true;
        }
        if (isEligible) {
