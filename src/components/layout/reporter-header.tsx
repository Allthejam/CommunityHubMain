 ...
 * Correctly scoped for static pre-rendering
 */
...
export default function ReporterHeader() {
  ...
  return (
    <header className={cn("sticky top-0 z-30 flex h-16 items-center gap-4 border-b bg-card px-4 sm:px-6")}>
      {isClient && <MobileNav menuItems={reporterNavItems as any} />}
      <Link href="/reporter/dashboard" className="flex items-center gap-2 font-semibold">
        <Logo className="h-6 w-6" />
        <span className="hidden sm:inline-block">Reporter Panel</span>
      </Link>
