...
export default function BusinessHeader() {
  ...
  const hasLeadershipRole = React.useMemo(() => {
    if (!userProfile) return false;
    if (userProfile.role === 'president' || userProfile.role === 'leader') return true;
    if (userProfile.communityRoles && Object.keys(userProfile.communityRoles).length > 0) {
      return Object.values(userProfile.communityRoles).some((role: any) => role.role === 'president' || role.role === 'leader');
    }
    return false;
  }, [userProfile]);
  
  const showAdminDashboard = ['admin', 'owner'].includes(userProfile?.role || '');
  const showLeaderDashboard = hasLeadershipRole || showAdminDashboard;
