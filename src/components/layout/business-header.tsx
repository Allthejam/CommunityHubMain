 ...
 * Moved outside component to ensure availability during pre-render
 */
...
const businessNavItems = [
  ...
  { href: '/business/storefront', label: 'Storefront', icon: Store, permission: 'canManageStorefront' },
  { href: '/business/team', label: 'Team', icon: UsersIcon, permission: 'canManageTeam' },
  { href: '/business/adverts', label: 'My Adverts', icon: Megaphone, permission: 'canManageAdverts' },
  { href: '/business/events', label: 'My Events', icon: Calendar, permission: 'canManageEvents' },
  { href: '/business/gallery', label: 'Gallery', icon: GalleryHorizontal, permission: 'canEditProfile' },
  { href: '/business/billing', label: 'Billing', icon: CreditCard, permission: 'canManageBilling' },
];
...
export default function BusinessHeader() {
  ...
  return (
    <header className={cn("sticky top-0 z-30 flex h-16 items-center gap-4 border-b bg-card px-4 sm:px-6")}>
      {isClient && <MobileNav menuItems={businessNavItems as any} />}
      <Link href="/business/dashboard" className="flex items-center gap-2 font-semibold">
        <Logo className="h-6 w-6" />
        <span className="hidden sm:inline-block">Business Panel</span>
      </Link>
