...
const engageSubItems = [
    { href: '/communities', label: 'Community Map', icon: MapIcon },
    { href: '/forum', label: 'Forum', icon: UsersIcon },
    { href: '/jobs', label: 'Jobs', icon: Briefcase },
    { href: '/marketplace', label: 'Buy, Swap & Sell', icon: ShoppingCart },
...
export default function AppHeader() {
  ...
  // DECLARE ALL STATE AND MEMOIZED LOGIC AT THE TOP TO PREVENT REFERENCE ERRORS
  ...
  const handleClaimLeadership = async () => {
    ...
    setIsSwitching(true);
    const result = await runAddCommunityToLeadership({ userId: user.uid, communityId: vId });
    if (result.success) {
